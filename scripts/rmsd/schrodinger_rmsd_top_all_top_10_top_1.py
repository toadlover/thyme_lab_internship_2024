		...
		if os.path.isfile("../../schrodinger_glide/binding_poses/" + dire + "-poses.mol2"):
			...
			for line in pose_file.readlines():
				...
				if "@<TRIPOS>BOND" in line:
					...
					#determine for the best 10 and best 1 as well
					#top 1
					if pose_counter == 1:
						best_pose_1 = pose_counter
						best_rmsd_1 = rmsd

					#top 10
					if pose_counter <= 10 and rmsd < best_rmsd_10:
						best_pose_10 = pose_counter
						best_rmsd_10 = rmsd
			...
			best_10_rmsd_dict[dire] = str(best_rmsd_10) + "," + str(best_pose_10) + "," + str(pose_counter)
			best_1_rmsd_dict[dire] = str(best_rmsd_1) + "," + str(best_pose_1) + "," + str(pose_counter)
			...
			best_10_rmsd_dict[dire] = "NA,NA,NA"
			best_1_rmsd_dict[dire] = "NA,NA,NA"
...
for key in best_10_rmsd_dict.keys():
	best_rmsd_list_10.append(str(key) + "," + best_10_rmsd_dict[key])

best_rmsd_list_1 = []

for key in best_1_rmsd_dict.keys():
	best_rmsd_list_1.append(str(key) + "," + best_1_rmsd_dict[key])
...
#10
rmsd_file = open("../../schrodinger_glide/glide_best_10_rmsd.csv", "w")

#header line for output file
rmsd_file.write("system_name,best_rmsd,best_rmsd_pose,total_poses\n")

for lig in sorted(best_rmsd_list_10):
	rmsd_file.write(lig + "\n")

rmsd_file.close()

#1
rmsd_file = open("../../schrodinger_glide/glide_best_1_rmsd.csv", "w")

#header line for output file
rmsd_file.write("system_name,best_rmsd,best_rmsd_pose,total_poses\n")

for lig in sorted(best_rmsd_list_1):
	rmsd_file.write(lig + "\n")
