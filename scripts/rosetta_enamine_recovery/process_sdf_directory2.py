...
# Define the base directory where you have write permissions
base_directory = "/home/raheel.sarwar-umw"  # Adjust this path as needed

# Define the path to the conformator tool and the license (adjust paths as needed)
conformator_path = "/Applications/conformator.app/Contents/MacOS/conformator"
license_key = "AAAAAAAljJkAAAAU2cp8rcvNNqldcfsfdZuLLxhvtpE="
...
def process_sdf(starting_file):
    ...
    # Make a directory under base_directory to perform all operations in and then move into it
    work_directory = os.path.join(base_directory, starting_file_no_ext)
    os.makedirs(work_directory, exist_ok=True)
    subprocess.run(["mv", starting_file, work_directory])
    os.chdir(work_directory)

    try:
        # Activate conformator
        result = subprocess.run([conformator_path, "--license", license_key], capture_output=True, text=True)
        print(result.stdout)
        print(result.stderr)

        # Run conformator on the starting file
        result = subprocess.run([conformator_path, "-i", starting_file, "-o", "confs.sdf", "--keep3d", "--hydrogens", "-n", "15", "-v", "0"], capture_output=True, text=True)
        print(result.stdout)
        print(result.stderr)

        # Check if confs.sdf was created
        if not os.path.exists("confs.sdf"):
        ...
        result = subprocess.run(["obabel", "confs.sdf", "-O", "individual_conf.sdf", "-m"], capture_output=True, text=True)
        print(result.stdout)
        print(result.stderr)
        ...
        print(f"Error processing {starting_file}: {e}")
